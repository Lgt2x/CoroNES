...
uint16_t CPU_6502::getAddress_c1(uint8_t mode) {
  ...
  switch (mode) {
  case X_IND:
    // Pre-indexed Indirect : return the byte at address 0xYYXX where XX is the
    // byte stored at (operand+X) and YY the byte at (operand+X+1)
