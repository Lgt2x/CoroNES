...
enum opcode_c0_b0_implied {
  BRK, JSR, RTI, RTS
};
enum opcode_c0_b2_implied {
  PHP, PLP, PHA, PLA, DEY, TAY, INY, INX
};
enum opcode_c0_b6_implied {
  CLC, SEC, CLI, SEI, TYA, CLV, CLD, SED
};
enum opcode_c0 {
  BIT = 1, JMP_abs, JMP_ind, STY, LDY, CPY, CPX
};
enum opcode_c0_branch {
...
enum adressingModes_c1 {
  X_IND, ZPG, IMM, ABS, IND_Y, ZPG_X, ABS_Y, ABS_X
};
enum opcode_c1 {
  ORA, AND, EOR, ADC, STA, LDA, CMP, SBC
};
enum flags {
  N_f, V_f, B_f, D_f, I_f, Z_f, C_f
};
...
void CPU_6502::reset() {
    ...
    PC = reset_vector;
    flags = std::vector<bool>{false, false, true, true, false, true, false, false};

    std::cout << "Reset, PC = " << PC << std::endl;
}
        ...
        case 0b01:
            switch (instruction) {
                ...
                case ADC: {  // Add memory to accumulator with carry
                    ...
                    // Set the overflow (V_f) flag. Is it set when the result changed the sign bit when it should not have.
                    // e.g. when both input numbers have the sign bit off but the result has the sign bit on.
                    // There is probably a more efficient way to do this.
                    // See http://www.6502.org/tutorials/vflag.html for full explanation
...
std::vector<uint8_t> CPU_6502::dumpRegisters() {
    ...
    return std::vector<uint8_t>{this->A, this->X, this->Y, this->SP, CPU_6502::flagsToByte(), static_cast<uint8_t>(this->PC & 0xFF),
                                static_cast<uint8_t>((uint8_t) this->PC >> 8)};
}
...
void CPU_6502::print_state() const {
    std::cout << "A=$" << print_hex(this->A)
              ...
              << " flags=0b" << std::bitset<8>{42} << (int) this->flagsToByte() << std::endl;
}
